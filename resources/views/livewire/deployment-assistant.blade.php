    ...
    <div x-show="expanded" x-transition:enter="transition ease-out duration-200"
        ...
        @if (filled($pendingProposal))
            <div class="px-4 py-3 border-t border-neutral-200 dark:border-coolgray-200">
                @if (data_get($pendingProposal, 'kind') === 'deployment_proposal')
                    ...
                    <dl class="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-xs dark:text-neutral-300 text-gray-700">
                        ...
                        {{-- Optional settings Shipbot may send; each is rendered
                             only when present so the user confirms exactly what
                             will be created. --}}
                        @foreach ([
            'name' => 'Name',
            'description' => 'Description',
            'domains' => 'Domain',
            'docker_compose_location' => 'Compose file',
            ...
            'start_command' => 'Start command',
                            ...
                            @if (filled(data_get($pendingProposal, $key)))
                                <dt class="font-medium">{{ $label }}</dt>
                                <dd class="font-mono break-all">{{ data_get($pendingProposal, $key) }}</dd>
                            @endif
                        @endforeach
                    </dl>
